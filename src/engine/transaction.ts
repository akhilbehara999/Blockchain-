 ...
 * @param options - Optional settings (e.g. skipRateLimit)
 * @returns Signed Transaction object
 * @throws Error if validation fails or rate limit exceeded
 */
export function createTransaction(from: string, to: string, amount: number, privateKey: string, fee: number = 0, options?: { skipRateLimit?: boolean }): Transaction {
  if (!options?.skipRateLimit && !txRateLimiter.canProceed()) {
